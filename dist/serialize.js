...
export default function serialize(data, schema) {
    const R = [];
    switch (schema.type) {
        case "null":
            R.push(0xC0);
            break;
        case "boolean":
            assertBoolean(data) && R.push(data ? 0xC3 : 0xC2);
            break;
        case "uint8":
            if (!assertNumber(data))
                break;
            if (schema.short)
                R.push(data);
            else
        ...
        case "int16":
            if (!assertNumber(data))
                break;
            if (data < 0)
                R.push(0xD1, ...getComplement2(destructNumber(-data, 2)));
            else
                R.push(0xD1, ...destructNumber(data, 2));
            break;
        case "int32":
            if (!assertNumber(data))
                break;
            if (data < 0)
                R.push(0xD2, ...getComplement2(destructNumber(-data, 4)));
            else
                R.push(0xD2, ...destructNumber(data, 4));
            break;
        case "int64":
            if (!assertBigInt(data))
                break;
            if (data[0] === "-")
                R.push(0xD3, ...getComplement2(destructBigInt(-BigInt(data), 8)));
            else
                R.push(0xD3, ...destructBigInt(BigInt(data), 8));
            break;
        case "float32":
            {
                if (!assertNumber(data))
        ...
        case "map8":
            {
                ...
                R.push(128 | entries.length);
                for (let i = 0; i < entries.length; i++) {
                    const [key, value] = entries[i];
                    for (const v of serialize(key, schema.entries[i][0]))
                        R.push(v);
                    for (const v of serialize(value, schema.entries[i][1]))
                        R.push(v);
                }
            }
            break;
